...
package com.apache.fastandroid.demo.component.materialdialogs
...
class MaterialDialogDemoActivity : BaseVBActivity<ActivityMaterialDialogsBinding>(ActivityMaterialDialogsBinding::inflate) {
  ...
  override fun layoutInit(savedInstanceState: Bundle?) {
    ...
    R.id.basic.onClickDebounced {
      MaterialDialog(this).show {
        ...
        debugMode(debugMode)
        lifecycleOwner(this@MaterialDialogDemoActivity)
      }
    }
  ...
  private fun showFileChooser() = runWithPermissions(READ_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      fileChooser(this@MaterialDialogDemoActivity) { _, file ->
  ...
  private fun showFileChooserButtons() = runWithPermissions(WRITE_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      fileChooser(context = this@MaterialDialogDemoActivity, allowFolderCreation = true) { _, file ->
        toast("Selected file: ${file.absolutePath}")
      }
      negativeButton(android.R.string.cancel)
      positiveButton(R.string.select)
  ...
  private fun showFileChooserFilter() = runWithPermissions(READ_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      fileChooser(context = this@MaterialDialogDemoActivity, filter = { it.extension == "txt" }) { _, file ->
        toast("Selected file: ${file.absolutePath}")
      }
      debugMode(debugMode)
      lifecycleOwner(this@MaterialDialogDemoActivity)
  ...
  private fun showFolderChooserButtons() = runWithPermissions(WRITE_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      folderChooser(context = this@MaterialDialogDemoActivity, allowFolderCreation = true) { _, folder ->
  ...
  private fun showFolderChooserFilter() = runWithPermissions(READ_EXTERNAL_STORAGE) {
    MaterialDialog(this).show {
      folderChooser(
          context = this@MaterialDialogDemoActivity, filter = { it.name.startsWith("a", true) }) { _, folder ->
        toast("Selected folder: ${folder.absolutePath}")
      }
      debugMode(debugMode)
      lifecycleOwner(this@MaterialDialogDemoActivity)
  ...
  override fun onCreateOptionsMenu(menu: Menu): Boolean {
    menuInflater.inflate(R.menu.menu_material_dialogs, menu)
