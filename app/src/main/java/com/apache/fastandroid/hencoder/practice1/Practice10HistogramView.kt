...
class Practice10HistogramView @JvmOverloads constructor(
    ...
    private var mCoorinateLineY: Float = 0f


    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        super.onSizeChanged(w, h, oldw, oldh)
        //x轴的 Y坐标
        mCoorinateLineY = height - 2 * MARGIN_BOTTOM
        NLog.d(TAG, "onSizeChanged w: %s, h:%s, padding: %s", w, h, paddingLeft)
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        mPath.reset()
        //画坐标系
        mPath.lineTo(0f, mCoorinateLineY)

        mPath.lineTo(width.toFloat(), mCoorinateLineY)
        ...
        for ((index, item) in mDatas.withIndex()) {
            ...
            mRect.set(left, (1 - item.value / 100f) * (mCoorinateLineY - RESERVE_WIDTH), right, mCoorinateLineY)
            //画柱状图
            ...
            val textY = mCoorinateLineY +  TEXT_RECT_GAP + bounds.height()
