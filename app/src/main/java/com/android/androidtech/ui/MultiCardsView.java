...
public class MultiCardsView extends View{
    ...
    public void onDraw(Canvas canvas) {
        ...
        Logger.d(String.format("clip bounds %d %d %d %d", clip.left, clip.top, clip.right, clip.bottom));
...
//            saveCount = canvas.save(Canvas.CLIP_SAVE_FLAG);
            ...
            if (canvas.clipRect(card.area)) {
                Logger.d("draw", "overdraw opt: draw cards index: " + index);
                Rect clip = canvas.getClipBounds();
                Logger.d("draw", String.format("current clip bounds %d %d %d %d", clip.left, clip.top, clip.right, clip.bottom));
                card.draw(canvas);
            }
            canvas.restoreToCount(saveCount);
        }else{
