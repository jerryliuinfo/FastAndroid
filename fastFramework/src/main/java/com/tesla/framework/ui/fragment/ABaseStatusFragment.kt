 ...
 * 封装 加载失败、网络异常、内容为空等异常视图处理
 */
open abstract class ABaseStatusFragment:ABaseFragment() {
    /**
     * Activity中由于服务器异常导致加载失败显示的布局。
     */
    var loadErrorView: View? = null
    ...
    private var mContentEmpty: Boolean = true
    ...
    open fun isContentEmpty():Boolean{
        return mContentEmpty;
    }

    fun setContentEmpty(result:Boolean){
        mContentEmpty = result
    }
