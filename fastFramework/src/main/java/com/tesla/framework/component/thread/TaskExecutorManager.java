...
public class TaskExecutorManager {
    private static volatile TaskExecutorManager sTaskExceutorManager;
    //CPU 密集型任务的线程池
    ...
    private final RejectedExecutionHandler mHandler = (r, executor) -> Executors.newCachedThreadPool().execute(r);

    public static TaskExecutorManager getInstance() {
        if (sTaskExceutorManager == null) {
            synchronized (TaskExecutorManager.class) {
                if (sTaskExceutorManager == null) {
                    sTaskExceutorManager = new TaskExecutorManager();
                }
            }
        }
